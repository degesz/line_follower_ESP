...
void motor_setup(){
    ...
    mcpwm_gpio_init(MCPWM_UNIT_0, MCPWM0A, 47);  // PWM A on GPIO 10
    mcpwm_gpio_init(MCPWM_UNIT_0, MCPWM0B, 21);  // Complementary PWM B on GPIO 11
    
    // PWM 2 on GPIO 21 and 33
    mcpwm_gpio_init(MCPWM_UNIT_1, MCPWM0A, 10);  // PWM A on GPIO 21
    mcpwm_gpio_init(MCPWM_UNIT_1, MCPWM0B, 11);  // Complementary PWM B on GPIO 33
    ...
    // Configure MCPWM for unit 1 (pins 21, 33)
...
void motors_set_pwm_frequency(uint32_t frequency) {
    ...
    // Configure MCPWM for unit 1 (pins 21 and 33)
    mcpwm_config_t pwm_config_2;
    pwm_config_2.frequency = frequency;           // Set the frequency dynamically
    pwm_config_2.counter_mode = MCPWM_UP_COUNTER; // Count up mode
    pwm_config_2.duty_mode = MCPWM_DUTY_MODE_0;   // Active high duty
    mcpwm_init(MCPWM_UNIT_1, MCPWM_TIMER_0, &pwm_config_2);  // Init MCPWM for unit 1 (pins 21, 33)
}